...
export const DEFAULT_VERIFICATION_E2E_IMAGE =
  'node:22.17.0-alpine3.22@sha256:fc3e945f920b7e3000cd1af86c4ae406ec70c72f328b667baf0f3a8910d69eed'
...
const pinnedImageSchema = z.string().trim().regex(
  /@sha256:[a-f0-9]{64}$/u,
  'must pin an OCI image by sha256 digest',
)
...
const verificationGateSchema = z.object({
  ...
  e2e: z.object({
    image: pinnedImageSchema.default(DEFAULT_VERIFICATION_E2E_IMAGE),
  ...
  load: z.object({
    ...
    timeoutSeconds: z.number().int().min(1).max(60 * 60).default(5 * 60),
    k6Image: pinnedImageSchema.optional(),
  }).strict(),
