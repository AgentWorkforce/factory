...
import type { VerificationTargetEnvironment, KubernetesEnvironmentTarget } from '../ports/environment.js'
import {
  type CommandRunner,
  helmConnectionArgs,
...
export interface PortForwarder {
  forward(input: {
    connection: KubectlConnectionOptions
    namespace: string
    service: string
...
function kubernetesTarget(environment: VerificationTargetEnvironment): KubernetesEnvironmentTarget {
  ...
  throw new StackDeploymentError(
    `VerificationTargetEnvironment ${environment.id} is not a Kubernetes environment with a namespace`,
    'apply',
  )
}
