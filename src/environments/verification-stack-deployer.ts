...
import type { VerificationEnvironment } from '../ports/environment.js'
import {
  type CommandRunner,
  helmConnectionArgs,
...
import { parseKubernetesResources, type KubernetesResource } from './kubernetes-client.js'
import { verificationKubernetesConnection } from './kubernetes-environment.js'
import { enforceKubernetesResourceSafety } from './kubernetes-provider.js'

type KubernetesEnvironmentTarget = KubernetesConnection & { namespace: string }
import {
...
export class VerificationStackDeployer {
  ...
  private async applySource(
    ...
    try {
      ...
      if (source.type === 'manifests') {
        rendered = (await Promise.all(source.paths.map(async (path) =>
          await readFile(resolveVerificationStackAsset(rootDir, path), 'utf8'),
        ))).join('\n---\n')
      } else if (source.type === 'kustomize') {
        rendered = (await this.runner.run('kubectl', [
          ...kubectlConnectionArgs(target),
          'kustomize', resolveVerificationStackAsset(rootDir, source.path),
        ], { timeoutMs: 120_000, signal })).stdout
      } else if (source.type === 'helm') {
        const chart = isRemoteChart(source.chart)
          ? source.chart
          : resolveVerificationStackAsset(rootDir, source.chart)
        rendered = (await this.runner.run('helm', [
          ...helmConnectionArgs(target),
          'template', source.release ?? descriptor.name, chart,
          '--namespace', target.namespace,
          '--include-crds',
          ...source.valuesFiles.flatMap((path) => [
            '--values', resolveVerificationStackAsset(rootDir, path),
          ]),
        ], { timeoutMs: 180_000, signal })).stdout
      } else {
        rendered = (await this.runner.run('kompose', [
          'convert', '--file', resolveVerificationStackAsset(rootDir, source.path), '--stdout',
        ], { timeoutMs: 120_000, signal })).stdout
      }

      const resources = this.safeResources(rendered, target)
      if (resources.length === 0) throw new Error(`${source.type} source rendered no Kubernetes resources`)
      await this.runner.run('kubectl', [
        ...kubectlConnectionArgs(target),
        '--namespace', target.namespace,
        'apply', '-f', '-',
  ...
  private safeResources(source: string, target: KubernetesEnvironmentTarget): KubernetesResource[] {
    return enforceKubernetesResourceSafety(parseKubernetesResources(source), {
      namespace: target.namespace,
      environmentId: target.namespace,
      connection: verificationKubernetesConnection({
        repository: 'verification-gate',
        context: target.context,
        kubeconfig: target.kubeconfig,
      }),
      descriptorAllowsClusterScope: false,
    })
  }
    ...
    try {
      if (seed.type === 'job') {
        const rendered = await readFile(
          resolveVerificationStackAsset(rootDir, seed.manifest),
          'utf8',
        )
        const resources = this.safeResources(rendered, target)
        if (!resources.some((resource) =>
          resource.kind === 'Job' && resource.metadata?.name === seed.job
        )) {
          throw new Error(`seed manifest does not define declared Job/${seed.job}`)
        }
        await this.runner.run('kubectl', [
          ...kubectlConnectionArgs(target),
          '--namespace', target.namespace,
...
function kubernetesTarget(environment: VerificationEnvironment): KubernetesEnvironmentTarget {
  return {
    namespace: environment.namespace,
    ...(environment.kubeContext ? { context: environment.kubeContext } : {}),
  }
}
