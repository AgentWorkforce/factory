...
import {
  ...
  type CommandRunner,
  type KubectlConnectionOptions,
  ProcessCommandRunner,
  kubectlConnectionArgs,
} from './kubernetes-command.js'

export interface KubernetesVerificationEnvironmentProviderOptions extends KubectlConnectionOptions {
...
/** Namespace-only Kubernetes target provider used by the composed verification gate. */
export class KubernetesVerificationEnvironmentProvider implements VerificationTargetProvider {
  private readonly runner: CommandRunner
  private readonly namespacePrefix: string
  private readonly defaultTtl: number
  private readonly maxActiveEnvironments: number
  private readonly connection: KubectlConnectionOptions
  private readonly environments = new Map<string, VerificationTargetEnvironment>()

  constructor(options: KubernetesVerificationEnvironmentProviderOptions = {}) {
  ...
  async provision(spec: VerificationTargetSpec = {}): Promise<VerificationTargetEnvironment> {
    ...
    const environment: VerificationTargetEnvironment = {
      id,
      status: 'ready',
