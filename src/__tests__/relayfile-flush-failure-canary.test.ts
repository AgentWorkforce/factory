...
describe('Relayfile flush-failure live canary', () => {
  ...
  it.each([
    ...
    ['incomplete timing evidence', {
      step: { stepStatus: { status: 'failed', error: 'payload_too_large' }, lastError: { type: 'RelayfileFlushError' }, failedAtMs: 1_000 },
      readinessReconcile: { state: 'retrying', lastError: 'RelayfileFlushError' },
    }],
    ['observation before failure', {
      step: { stepStatus: { status: 'failed', error: 'payload_too_large' }, lastError: { type: 'RelayfileFlushError' }, failedAtMs: 2_000 },
      readinessReconcile: { state: 'failed', lastError: 'RelayfileFlushError', lastFailureAtMs: 1_000, intervalMs: 60_000 },
    }],
