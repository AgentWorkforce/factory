...
import { ConnectionGithubWriteback, GhCliGithubWriteback, MountGithubRead, MountLinearWriteback, MountSlackWriteback, slackChannelAliases, slackChannelSegment, supportsConnectionGithubWriteback } from '../writeback'
...
const unavailableGithubAppWriteback = (githubWrite: GithubConnectionWrite | undefined): GithubWriteback => {
  const missing = githubWrite
    ? supportsConnectionGithubWriteback.missing(githubWrite)
    : ['workspace GitHub connection']
  const refuse = (): never => {
    throw new Error(
      `GitHub lifecycle write requires the connected Factory app (missing ${missing.join(', ')}); ` +
      'refusing to fall back to the local gh user',
    )
  }
  return {
    postComment: async () => refuse(),
    setStatus: async () => refuse(),
    closeIssue: async () => refuse(),
  }
}
...
export class FactoryLoop implements Factory {
  ...
  readonly #githubUserPullRequestPublisher: GithubPullRequestPublisher
  ...
  constructor(config: FactoryConfig, ports: FactoryPorts) {
    ...
    const githubAppWrite = supportsConnectionGithubWriteback(ports.mount.githubWrite)
      ? ports.mount.githubWrite
      : undefined
    // Issue lifecycle and babysitter writes never fall back to `gh`: doing so
    // would silently use the human logged in on this host. A missing/older
    // connection fails explicitly only when a GitHub mutation is attempted, so
    // Linear-only workspaces can still start.
    this.#githubWriteback = ports.githubWriteback
      ?? (githubAppWrite
        ? new ConnectionGithubWriteback({ githubWrite: githubAppWrite, mount: ports.mount })
        : unavailableGithubAppWriteback(ports.mount.githubWrite))
    const userPullRequestWriteback = ports.githubWriteback?.publishPullRequest
      ? ports.githubWriteback
      : new GhCliGithubWriteback()
    this.#githubUserPullRequestPublisher = {
      publishPullRequest: userPullRequestWriteback.publishPullRequest!.bind(userPullRequestWriteback),
    }
    ...
    this.#mergeGate = ports.mergeGate ?? new GithubMergeGate(ports.mount)
  ...
  #githubPullRequestPublisher(): {
    ...
    return {
      identity: 'user',
      publisher: this.#githubUserPullRequestPublisher,
    }
  }
