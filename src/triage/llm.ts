...
export function buildPrompt(issue: LinearIssue, ctx: TriageContext): string {
  return [
    ...
    '- Execution shape labels are orthogonal to repo labels and win over inference: agent:single => single, agent:workflow => workflow, agent:team => team.',
    '- Thin issue: description under 140 characters or no acceptance signal.',
    '- Without an execution shape label, infer single, workflow, or team. Use workflow for issues that describe an existing multi-step workflow or cross-agent orchestration; use team iff two or more surface buckets are named, or two or more routes are needed. Use at most two implementers.',
    '- Workflow scope must include one workflow role with capability workflow:run, workflow workflows/factory/linear-issue.ts, inputs, and no implementers.',
    '- Preserve Linear agent naming: ar-{n}-impl, ar-{n}-impl-{scope-or-repo}, ar-{n}-workflow, ar-{n}-review.',
    '- For GitHub-native issues, append the sanitized source repo basename to every broker-global role: ar-{n}-impl-{repo}, ar-{n}-workflow-{repo}, ar-{n}-review-{repo}.',
